                ...
                if vRP.tryPayment(user_id, price) then
                  if price > 0 then
                    if custom[1][1] >= 0 then
                      if custom[1][1] ~= old_custom[1][1] or custom[1][1] == old_custom[1][1] and custom[1][2] ~= old_custom[1][2] then
                        vRP.setSData("ped:u" .. user_id .. "mask", json.encode({custom[1][1], custom[1][2]}))
                      end
                      if vRP.getInventoryItemAmount(user_id, "mascara") <= 0 then
                        vRP.giveInventoryItem(user_id, "mascara", 1, true)
                      end
                    end
...
local function build_client_skinshops(source)
  ...
  if user_id ~= nil then
    for k, v in pairs(skinshops) do
      ...
      vRPclient.addScaledBlip(source, {x, y, z, 0.7, 73, 4, lang.skinshop.title()})
      vRPclient.addMarker(source, {x, y, z, 1.5, 1.5, 0.5, 0, 255, 125, 125, 150})
